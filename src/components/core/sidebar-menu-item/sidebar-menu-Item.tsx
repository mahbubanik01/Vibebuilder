...
export const SidebarMenuItemComponent = ({
  ...
  return (
    <SidebarMenuItem className="relative">
      {isActive && (
        <div className="absolute left-[-12px] top-1 bottom-1 w-[4px] premium-gradient rounded-r-full z-10" />
      )}
      <SidebarMenuButton asChild className={`${isActive ? 'bg-blue-50/50' : 'hover:bg-gray-50'} transition-all duration-200`}>
        <Link to={item.path} className="flex items-center w-full relative" onClick={handleClick}>
          {renderIcon(item.icon as MenuIconName)}
          <span
            className={`ml-3 truncate ${!showText && 'hidden'} ${isActive ? 'text-blue-600 font-bold' : 'text-high-emphasis'} text-base`}
          >
            {t(item.name)}
          </span>
          {shouldShowAdminBadge && showText && (
