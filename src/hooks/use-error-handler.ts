...
export const useErrorHandler = (defaultOptions: ErrorHandlerOptions = {}) => {
  ...
  const formatErrorMessage = (
    ...
    if (typeof errorMessage === 'object' && errorMessage !== null) {
      try {
        const str = JSON.stringify(errorMessage);
    ...
    const isTranslationKey = typeof errorMessage === 'string' && TRANSLATION_KEY_PATTERN.test(errorMessage);
    return translate && isTranslationKey ? t(errorMessage) : String(errorMessage ?? 'An error occurred');
  };
