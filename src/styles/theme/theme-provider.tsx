 ...
 * supporting light, dark, and system themes with browserStorage persistence.
 *
 * Features:
 * - Theme state management (light, dark, system)
 * - Persistent theme selection using browserStorage
 ...
 * @param {string} [storageKey='theme'] - The browserStorage key used to persist theme preference
 *
 * @example
...
export function ThemeProvider({
  ...
  const [theme, setTheme] = useState<Theme>(
    () => (browserStorage.getItem(storageKey) as Theme) || defaultTheme
  );

  const [colors, setColors] = useState(() => {
    const themeColors = getThemeColors();
    const currentTheme = theme === 'dark' ? themeColors.dark : themeColors.light;
    const defaultPrimary = import.meta.env.VITE_PRIMARY_COLOR || '#2F81F7';
    const defaultSecondary = import.meta.env.VITE_SECONDARY_COLOR || '#30363D';
  ...
  useEffect(() => {
    ...
    setColors({
      primary: import.meta.env.VITE_PRIMARY_COLOR || '#2F81F7',
      secondary: import.meta.env.VITE_SECONDARY_COLOR || '#30363D',
    });
  }, [theme]);
  ...
  const value = useMemo(
    () => ({
      ...
      setTheme: (newTheme: Theme) => {
        browserStorage.setItem(storageKey, newTheme);
        setTheme(newTheme);
      },
    }),
