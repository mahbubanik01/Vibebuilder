...
const setTempEventToStorage = (eventData: any): void => {
  try {
    browserStorage.removeItem(TEMP_EVENT_KEY);
    browserStorage.setItem(TEMP_EVENT_KEY, JSON.stringify(eventData));
  } catch (error) {
    ...
    try {
      ...
      browserStorage.setItem(TEMP_EVENT_KEY, JSON.stringify(minimalData));
    } catch (e) {
...
export const AddEvent = ({ start, end, onCancel, onSubmit }: Readonly<AddEventProps>) => {
  ...
  const handleRecurrenceClose = () => {
    ...
    const tempRecurringEvents = browserStorage.getItem(TEMP_RECURRING_EVENTS_KEY);
