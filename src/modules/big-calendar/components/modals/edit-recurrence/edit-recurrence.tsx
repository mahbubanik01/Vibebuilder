...
export const EditRecurrence = ({ event, onNext, setEvents }: Readonly<EditRecurrenceProps>) => {
  ...
  const [initialRecurrenceEvent] = useState<CalendarEvent>(() => {
    if (typeof window !== 'undefined') {
      const saved = browserStorage.getItem('tempEditEvent');
      if (saved) {
  ...
  const [period, setPeriod] = useState<string>(() => {
    ...
    // Then check browserStorage
    if (typeof window !== 'undefined') {
      const saved = browserStorage.getItem('tempRecurrenceSettings');
      if (saved) {
        try {
  ...
  // Save settings to browserStorage whenever they change
  useEffect(() => {
    const settings = {
      period,
      selectedDays,
    ...
    try {
      browserStorage.setItem('tempRecurrenceSettings', JSON.stringify(settings));
    } catch (error) {
  ...
  // Load initial settings from browserStorage if available
  useEffect(() => {
  ...
  useEffect(() => {
    const saved =
      typeof window !== 'undefined' && browserStorage.getItem('tempRecurringEvents');
  ...
  const handleSave = () => {
    ...
    try {
      ...
      browserStorage.removeItem('tempEditEvent');
      browserStorage.setItem('tempEditEvent', JSON.stringify(eventTemplate));
      ...
      // Save all events to browserStorage
      browserStorage.setItem('tempRecurringEvents', JSON.stringify(allEvents));
      ...
      try {
        ...
        browserStorage.setItem('tempEditEvent', JSON.stringify(minimalTemplate));
        ...
        if (firstEvent) {
          ...
          browserStorage.setItem('tempRecurringEvents', JSON.stringify(minimalEvents));
        }
  ...
  useEffect(() => {
    return () => {
      if (!browserStorage.getItem('tempRecurringEvents')) {
        browserStorage.removeItem('tempRecurringEvents');
        browserStorage.removeItem('tempEditEvent');
      }
    };
  }, []);
