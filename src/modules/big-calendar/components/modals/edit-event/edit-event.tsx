...
const loadRecurringEventsFromStorage = (initialEventData: CalendarEvent): CalendarEvent[] => {
  ...
  const tempEdit = browserStorage.getItem('tempEditEvent');
  if (tempEdit) {
    try {
  ...
  if (events.length < 2) {
    const tempRec = browserStorage.getItem('tempRecurringEvents');
    if (tempRec) {
      try {
...
export const EditEvent = ({
  ...
  useEffect(() => {
    const savedEventData = browserStorage.getItem('tempEditEvent');
  ...
  const handleFormSubmit = (data: AddEventFormValues) => {
    if (initialEventData.resource?.recurring) {
      ...
      browserStorage.setItem('tempUpdateData', JSON.stringify(data));
    } else {
      onSubmit(data);
    }
  };
  ...
  const handleUpdateConfirm = (updateOption: DeleteUpdateEventOption) => {
    const tempData = browserStorage.getItem('tempUpdateData');
    if (tempData) {
      const formData = JSON.parse(tempData) as AddEventFormValues;
      onSubmit(formData, updateOption);
      browserStorage.removeItem('tempUpdateData');
    }
  ...
  const handleRecurringClick = () => {
    ...
    browserStorage.setItem('tempEditEvent', JSON.stringify(tempEventData));
    onNext();
  };
