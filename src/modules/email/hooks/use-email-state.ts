...
export const useEmailState = (category?: string, emailId?: string) => {
  const [emails, setEmails] = useState<Record<string, TEmail[]>>(() => {
    try {
      const saved = browserStorage.getItem('emails-state');
      if (saved) {
        return JSON.parse(saved) as Record<string, TEmail[]>;
      }
    } catch {
      // noop: ignore JSON parse/browserStorage access errors
    }
  ...
  useEffect(() => {
    try {
      browserStorage.setItem('emails-state', JSON.stringify(emails));
    } catch {
      // noop: ignore browserStorage write errors
    }
  }, [emails]);
